...
const consys = <T>(
    ...
    const parser = (args: T) => {
        ...
        // const parseArgumentsInObject = (arg:any, item?:any) => {
        //     let argName:string = arg[0]
        //     let argValue:any = arg[1]
        ...
        //     if(configItem){
        //         log(`Found "${argName}" in config:`, config[argName])
                
        //         //$ Find and use the env variable if one is specified
        //         if(configItem.env && configItem.env !== ''){
        //             argValue = process?.env[configItem.env] || arg[1]
        //         }
        ...
        //             configItem.keys.forEach((key:string) => {
        //                 const nestKey = `$${key}`
        ...
        // Object.entries(args).forEach(parseArgumentsInObject)

        Object.entries(args).forEach((arg:any) => {
            parseArgumentsInObject(arg[0], arg[1], config[arg[0]])
        })
